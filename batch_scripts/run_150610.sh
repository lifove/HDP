date=20150610
postfix=_fs_s15
cofeature_prefix=20150610_FS_S15
server=`hostname`
numOfThreads=20
saveNewData=false
...
#do
	rawdata=$server/$date$postfix.txt
        java -Xmx13524m -cp  bin/CrossPredictor.jar$libs hk.ust.cse.ipam.jc.crossprediction.util.WinTieLossByUtest Results/$rawdata Results/skcpu7/AUC_from_common_features.csv $fsOption $outputRScript > Results/$server/$date\_wintieloss$postfix\_R_$outputRScript.txt
