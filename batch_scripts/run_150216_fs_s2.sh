date=20150216
postfix=_fs_s15
cofeature_prefix=20150216_FS_S15
server=`hostname`
numOfThreads=1
saveNewData=false
...
for cutoff in $cutoffs
do
	rawdata=$server/$date$postfix\_$cutoff.txt
        java -Xmx13524m -cp  bin/Tester.jar$libs hk.ust.cse.ipam.jc.crossprediction.util.WinTieLossByUtest Results/$rawdata Results/skcpu7/AUC_from_common_features.csv $fsOption $outputRScript > Results/$server/$date\_wintieloss$postfix\_$cutoff.txt
